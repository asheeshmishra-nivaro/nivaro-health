                            ...
                            <div className="flex-[3.5] flex flex-col bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden relative min-w-0">
                                ...
                                <div className="p-6 border-b border-slate-100 bg-slate-50/50 shrink-0">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="w-14 h-14 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-lg font-black shadow-xl shadow-indigo-600/20">
                                                {patient?.name?.[0]}
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-3 mb-1">
                                                    <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{patient?.name}</h2>
                                                    <div className="px-2 py-0.5 bg-emerald-500 text-white text-[7px] font-black rounded-md uppercase tracking-widest flex items-center gap-1 border border-emerald-500/20">
                                                        <ShieldCheck className="w-2.5 h-2.5" /> Verified Profile
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Age: {patient?.age} • Gender: {patient?.gender}</p>
                                                    <div className="w-1 h-1 rounded-full bg-slate-200" />
                                                    <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest flex items-center gap-1.5 underline underline-offset-4 decoration-indigo-500/30">
                                                        <Database className="w-3 h-3 text-indigo-400" /> Secure Medical Vault
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="bg-slate-50 p-1.5 rounded-xl border border-slate-100 flex items-center gap-4 px-4">
                                                <div className="flex flex-col">
                                                    <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Session</span>
                                                    <span className="text-xs font-mono font-black text-slate-900 leading-none">{formatTime(sessionTimer)}</span>
                                                </div>
                                                <Timer className="w-4 h-4 text-emerald-500 animate-spin-slow" />
                                            </div>

                                            <button
                                                onClick={() => setShowVideo(!showVideo)}
                                                className={cn(
                                                    "px-6 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.1em] flex items-center gap-3 transition-all active:scale-95 shadow-lg",
                                                    showVideo
